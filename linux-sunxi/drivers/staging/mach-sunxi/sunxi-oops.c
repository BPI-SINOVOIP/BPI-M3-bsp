...
#define OOPS_LOG_PARTITION "/dev/block/by-name/alog"

/*
 * klog partition have two part:
 *     |<-- android log(64M) -->|<-- kernel log(16M) -->|
 */
#define KERNEL_LOG_SIZE                                (16 * 1024 * 1024)
...
static void sunxi_oops_workfunc_read(struct work_struct *work)
{
        ...
        struct tm log_tm;
        loff_t partition_size, oops_base_addr_offset;
        ...
        oldfs = get_fs();
        set_fs(get_ds());
        // kernel log base addr = partition_size - kernel_log_size
        partition_size = vfs_llseek(filp, 0, SEEK_END);
        oops_base_addr_offset = partition_size - KERNEL_LOG_SIZE;
        if(sunxi_oops_info.oops_count <= OOPS_MAX_COUNT){
                vfs_llseek(filp, oops_base_addr_offset, SEEK_SET);
                ...
                vfs_llseek(filp, oops_base_addr_offset + BUF_LEN * sunxi_oops_info.write_pos, SEEK_SET);
...
void sunxi_oops_dump(void)
{
        ...
        partition_size = vfs_llseek(filp, 0, SEEK_END);
        oops_base_addr_offset = partition_size - KERNEL_LOG_SIZE;
        vfs_llseek(filp, oops_base_addr_offset + (loff_t)BUF_LEN*sunxi_oops_info.write_pos, SEEK_SET);
